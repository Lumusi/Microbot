...
public interface BankskillerConfig extends Config {
    ...
    enum StaminaDose {
        FOUR_DOSE(3016), // Super energy (4)
        THREE_DOSE(3018), // Super energy (3)
        TWO_DOSE(3020), // Super energy (2)
        ONE_DOSE(3022); // Super energy (1)
    ...
    @ConfigItem(keyName = "grimyHerbID", name = "Grimy Herb ID", description = "The item ID of the grimy herb to clean.", position = 0, section = herbCleaningSection)
    default int grimyHerbID() { return 0; }
    @ConfigItem(keyName = "minClickDelay", name = "Min Click Delay (ms)", description = "Minimum delay between each herb clean click.", position = 1, section = herbCleaningSection)
    default int minClickDelay() { return 50; }
    @ConfigItem(keyName = "maxClickDelay", name = "Max Click Delay (ms)", description = "Maximum delay between each herb clean click.", position = 2, section = herbCleaningSection)
    ...
    @ConfigItem(keyName = "itemID1", name = "Item 1 ID", description = "First ingredient. Set to 0 to ignore.", position = 0, section = itemCombinationSection)
    default int itemID1() { return 0; }
    @ConfigItem(keyName = "item1Amount", name = "Item 1 Amount", description = "Amount to withdraw. Set to 0 to ignore.", position = 1, section = itemCombinationSection)
    default int item1Amount() { return 0; }
    @ConfigItem(keyName = "itemID2", name = "Item 2 ID", description = "Second ingredient. Set to 0 to ignore.", position = 2, section = itemCombinationSection)
    default int itemID2() { return 0; }
    @ConfigItem(keyName = "item2Amount", name = "Item 2 Amount", description = "Amount to withdraw. Set to 0 to ignore.", position = 3, section = itemCombinationSection)
    default int item2Amount() { return 0; }
    @ConfigItem(keyName = "itemID3", name = "Item 3 ID", description = "Third ingredient. Set to 0 to ignore.", position = 4, section = itemCombinationSection)
    default int itemID3() { return 0; }
    @ConfigItem(keyName = "item3Amount", name = "Item 3 Amount", description = "Amount to withdraw. Set to 0 to ignore.", position = 5, section = itemCombinationSection)
    default int item3Amount() { return 0; }
    @ConfigItem(keyName = "itemID4", name = "Item 4 ID", description = "Fourth ingredient. Set to 0 to ignore.", position = 6, section = itemCombinationSection)
    default int itemID4() { return 0; }
    @ConfigItem(keyName = "item4Amount", name = "Item 4 Amount", description = "Amount to withdraw. Set to 0 to ignore.", position = 7, section = itemCombinationSection)
    default int item4Amount() { return 0; }

    @ConfigItem(keyName = "combinationOrder", name = "Use Item on Item (ID1,ID2)", description = "Action to perform. E.g., '269,2436' to use Torstol on Super Attack.", position = 8, section = itemCombinationSection)
    default String combinationOrder() { return ""; }
    @ConfigItem(keyName = "makeXAmount", name = "Make-X Option", description = "Option in Make-X interface. 0=Spacebar, 1=First option, etc.", position = 9, section = itemCombinationSection)
    default int makeXAmount() { return 0; }
    @ConfigItem(keyName = "noAnimation", name = "No Animation Mode", description = "Enable for actions with no animation (e.g., fletching darts).", position = 10, section = itemCombinationSection)
    default boolean noAnimation() { return false; }
    @ConfigItem(keyName = "noAnimationWait", name = "No Animation Wait (s)", description = "Time in seconds to wait for a full inventory action.", position = 11, section = itemCombinationSection)
